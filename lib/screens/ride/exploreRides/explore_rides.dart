  ...
  Future<void> _handleRefresh(UserModel user) async {
    try {
      ...
      try {
        ...
        Position? position =
            await widget.userState.getCurrentLocationWithPermissionHandling();
        currentLocation =
            LatLng(position?.latitude ?? 0, position?.longitude ?? 0);
      } catch (locationError) {
        // Handle location error - either show dialog or continue with null location
        debugPrint('Location error: $locationError');
        await widget.userState.showLocationPermissionDialog(context);
        // Continue with null location or previous location
      }
      ...
      setState(() {
        ...
        rideOfferCards = offers
            .map((offer) => RideOfferCard(
                  ...
                  rideOffer: offer,
                  currentLocation:
                      currentLocation, // This might be null if permission denied
                  refreshOffersIndicatorKey: refreshOffersIndicatorKey,
                ))
            .toList();
      });
...
class _RideOfferListState extends State<RideOfferList> {
  ...
  void _sortOffersByDistance() {
    ...
    sortedOffers.sort((a, b) {
      double distanceA = Geolocator.distanceBetween(
          ...
          a.rideOffer.driverLocation.latitude,
          a.rideOffer.driverLocation.longitude);
      double distanceB = Geolocator.distanceBetween(
          widget.currentLocation!.latitude,
          widget.currentLocation!.longitude,
          b.rideOffer.driverLocation.latitude,
          b.rideOffer.driverLocation.longitude);
  ...
  void _sortOffersByLeaveTime() {
    List<RideOfferCard> sortedOffers = List.from(widget.rideOfferCards);
    sortedOffers.sort((a, b) {
      return a.rideOffer.proposedLeaveTime!
          .compareTo(b.rideOffer.proposedLeaveTime!);
    });
    _rebuildOffers(sortedOffers);
  }
  ...
  void _sortOffersByBackTime() {
    List<RideOfferCard> sortedOffers = List.from(widget.rideOfferCards);
    sortedOffers.sort((a, b) {
      return a.rideOffer.proposedBackTime!
          .compareTo(b.rideOffer.proposedBackTime!);
    });
    _rebuildOffers(sortedOffers);
  }
